  ...
  data: [
    {
      ...
      name: '迭代完成率',
      desc:
        '指标业务价值：反映迭代中的故事按计划完成的能力，是衡量迭代团队管理和运作成熟度的参考指标。<br><br>计算公式：迭代完成率=∑实际完成故事点数/∑计划完成故事点数<br><br>详细逻辑：<br>1、实际完成故事点数：统计迭代结束日期后第3个工作日24:00，在看板价值列为测试完成及之后的故事卡的故事点数（不包括子卡故事点数）<br>注：若在迭代结束日期后第3个工作日24:00之前手动关闭迭代，则在迭代关闭时刻统计实际完成故事点数<br>2、计划完成故事点数：统计迭代激活且迭代开始日期后第5个工作日24:00，迭代内所有故事卡的故事点数（不包括子卡故事点数）',
      sort: '1',
      type: 'iteration1',
      isNew: true,
      url1: 'url1',
    ...
    },
    {
      ...
      name: '迭代速率',
      desc:
        '指标业务价值：度量各迭代的产出故事点数，通过趋势分析反映各迭代的交付容量变化。<br><br>计算公式：迭代速率=∑迭代完成故事点数/∑迭代个数<br><br>详细逻辑：<br>迭代完成故事点数：统计迭代结束日期后第3个工作日24:00，在看板价值列为测试完成及之后的故事卡的故事点数（不包括子卡故事点数）',
      type: 'iteration2',
      isNew: false,
      sort: '2',
      url1: 'url1',
    ...
    },
    {
      ...
      name: '报工统计',
      desc:
        '指标业务价值：反映敏捷团队的投入与资源的分布情况。<br><br>指标说明：统计敏捷团队的报工工时之和',
      type: 'iteration3',
      isNew: false,
      sort: '3',
      url1: 'url1',
...
const getTargetList = {
  ...
  template: config => {
    ...
    // 是否有分页条件，没有分页条件就返回全部
    if (pageSize && pageNum) {
      targetList = totalList.filter(
        (item, index) =>
          index < pageSize * pageNum && index >= pageSize * (pageNum - 1)
      )
    } else {
      targetList = totalList
    }
